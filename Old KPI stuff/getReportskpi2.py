...
def main():
    ...
    # make real dates
    try:
        systemStartDate = sentrykpi2.systemDateTime(results.startDate)
        systemEndDate = sentrykpi2.systemDateTime(results.endDate)
    except Exception, e:
    ...
    startDate = sentrykpi2.sentryDate(systemStartDate)
    startTime = sentrykpi2.sentryTime(systemStartDate)
    endDate = sentrykpi2.sentryDate(systemEndDate)
    endTime = sentrykpi2.sentryTime(systemEndDate)


    report = sentrykpi2.getReportskpi2(startDate=startDate,
             startTime=startTime,
             endDate=endDate,
             endTime=endTime,
             reportDuration=int(results.duration))

    sentryConnection = sentrykpi2.sentry( sentryIP=results.system,
                         username=results.userName,
                         passwd=results.password)

    print report.startDateTime,report.endDateTime
    for date in sentrykpi2.dateRange(report.startDateTime,report.endDateTime, report.reportDuration):

        logger.info("Start Date/Time: {0} {1}/End Date/Time: {2} {3}".format(sentrykpi2.sentryDate(date),
                                                                                sentrykpi2.sentryTime(date),
                                                                                sentrykpi2.sentryDate(date+timedelta(seconds = report.reportDuration)),
                                                                                sentrykpi2.sentryTime(date+timedelta(seconds = report.reportDuration))))
        stats = sentryConnection.getStats(fromdate=sentrykpi2.sentryDate(date),
                                            todate=sentrykpi2.sentryDate(date+timedelta(seconds = report.reportDuration)),
                                            fromtime=sentrykpi2.sentryTime(date),
                                            totime=sentrykpi2.sentryTime(date+timedelta(seconds = report.reportDuration)),
                                            preview="1",
                                            streamcsv="1") 
